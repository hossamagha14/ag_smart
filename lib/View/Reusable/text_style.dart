import 'package:ag_smart/View/Reusable/colors.dart';
import 'package:flutter/material.dart';

TextStyle mainIcon =
    TextStyle(fontSize: 30, fontFamily: 'icons', color: iconColor);

TextStyle yellowIcon =
    TextStyle(fontSize: 30, fontFamily: 'icons', color: yellowColor);

TextStyle bigIcon =
    TextStyle(fontSize: 55, fontFamily: 'icons', color: iconColor);

TextStyle smallIcon =
    TextStyle(fontSize: 18, fontFamily: 'icons', color: iconColor);

TextStyle smallIconOff =
    const TextStyle(fontSize: 18, fontFamily: 'icons', color: Colors.grey);

TextStyle smallIconOn =
    TextStyle(fontSize: 18, fontFamily: 'icons', color: yellowColor);


